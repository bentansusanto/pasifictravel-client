import { Document_and_Visa } from "@/utils/DocumentandVisa/Document_and_Visa";
import { PriceDetailProps } from "@/utils/DocumentandVisa/Document_and_Visa";
import { CountrySection } from "@/utils/DocumentandVisa/Document_and_Visa";

export const ListDocumentVisa: Document_and_Visa[] = [
  {
    Heading: "Lorem ipsum dolor",
    subHeading: "Lorem ipsum dolor sit amet, consectetur adipiscing elit,",
  },
  {
    Heading: "Lorem ipsum dolor",
    subHeading: "Lorem ipsum dolor sit amet, consectetur adipiscing elit,",
  },
  {
    Heading: "Lorem ipsum dolor",
    subHeading: "Lorem ipsum dolor sit amet, consectetur adipiscing elit,",
  },
];

export const PriceList: PriceDetailProps[] = [
  {
    tourists: 3,
    pricePerPerson: 1050100,
  },
];

export const ListCountry: CountrySection[] = [
  {
    headPicture: "switzerland.png",
    Desc: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.",
    Flag: "switzerland-flag.png",
    FlagName: "Switzerland",
    Capital_City: "Bern",
    DescWord:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.",
    id: 1,
    Type_VisaorPass: "Visa",
    type_Country: "Popular",
  },
  {
    headPicture: "canada.png",
    Desc: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.",
    Flag: "canada-flag.png",
    FlagName: "Canada",
    Capital_City: "Ottawa",
    DescWord:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.",
    id: 2,
    Type_VisaorPass: "Pasport",
    type_Country: "Popular",
  },
  {
    headPicture: "china.png",
    Desc: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.",
    Flag: "china-flag.png",
    FlagName: "China",
    Capital_City: "Beijing",
    DescWord:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.",
    id: 3,
    Type_VisaorPass: "Pasport",
    type_Country: "Tidak Popular",
  },
  {
    headPicture: "germany.png",
    Desc: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.",
    Flag: "germany-flag.png",
    FlagName: "Germany",
    Capital_City: "Berlin",
    DescWord:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.",
    id: 4,
    Type_VisaorPass: "Visa",
    type_Country: "Popular",
  },
  {
    headPicture: "germany.png",
    Desc: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.",
    Flag: "germany-flag.png",
    FlagName: "Germany",
    Capital_City: "Berlin",
    DescWord:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.",
    id: 5,
    Type_VisaorPass: "Visa",
    type_Country: "Popular",
  },
  {
    headPicture: "germany.png",
    Desc: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.",
    Flag: "germany-flag.png",
    FlagName: "Germany",
    Capital_City: "Berlin",
    DescWord:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.",
    id: 6,
    Type_VisaorPass: "Visa",
    type_Country: "Popular",
  },
  {
    headPicture: "germany.png",
    Desc: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.",
    Flag: "germany-flag.png",
    FlagName: "Germany",
    Capital_City: "Berlin",
    DescWord:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.",
    id: 7,
    Type_VisaorPass: "Visa",
    type_Country: "Popular",
  },
  {
    headPicture: "germany.png",
    Desc: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.",
    Flag: "germany-flag.png",
    FlagName: "Germany",
    Capital_City: "Berlin",
    DescWord:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.",
    id: 8,
    Type_VisaorPass: "Visa",
    type_Country: "Tidak Popular",
  },
  {
    headPicture: "switzerland.png",
    Desc: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.",
    Flag: "switzerland-flag.png",
    FlagName: "Switzerland",
    Capital_City: "Bern",
    DescWord:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.",
    id: 1,
    Type_VisaorPass: "Visa",
    type_Country: "Tidak Popular",
  },
  {
    headPicture: "canada.png",
    Desc: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.",
    Flag: "canada-flag.png",
    FlagName: "Canada",
    Capital_City: "Ottawa",
    DescWord:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.",
    id: 2,
    Type_VisaorPass: "Pasport",
    type_Country: "Popular",
  },
  {
    headPicture: "china.png",
    Desc: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.",
    Flag: "china-flag.png",
    FlagName: "China",
    Capital_City: "Beijing",
    DescWord:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.",
    id: 3,
    Type_VisaorPass: "Pasport",
    type_Country: "Tidak Popular",
  },
  {
    headPicture: "germany.png",
    Desc: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.",
    Flag: "germany-flag.png",
    FlagName: "Germany",
    Capital_City: "Berlin",
    DescWord:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.",
    id: 4,
    Type_VisaorPass: "Visa",
    type_Country: "Tidak Popular",
  },
];

export const listDocumentCountry = ["Visa", "Pasport", "Travel Insurance"];
export const sortingDocumentCountry = ["Popular"];
